...
class RedTableWrapper(BaseWrapper):
    ...
    def get_table(self,output_mode='red_table',success=None):
        if output_mode == 'red_table':
            return self._create_red_table(success=success)
        elif output_mode == 'true_table':
            return self.env.get_table()

    def observation_change(self, observation):
        red_obs = deepcopy(observation) if type(observation) == dict else deepcopy(observation.data)
        success = red_obs.pop('success')

        self.step_counter += 1
        if self.step_counter <= 0:
            self._process_initial_obs(red_obs)
        elif success:
            self._update_red_info(red_obs)

        if self.output_mode == 'table':
            return self._create_red_table(success)
    ...
    def _create_red_table(self, success):
    ...
    def _create_vector(self, success, num_hosts=13):
        table = self._create_red_table(success)._rows
        ...
        success_value = int(success.value) if success.value < 2 else -1
    ...
    def get_observation(self, agent: str):
        if agent == 'Red':
            output = self.get_table(self,agent)
        else:
